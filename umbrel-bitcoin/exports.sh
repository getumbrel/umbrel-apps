...
BIN_ARGS+=( "-zmqpubhashblock=tcp://0.0.0.0:${APP_UMBREL_BITCOIN_ZMQ_HASHBLOCK_PORT}" )
BIN_ARGS+=( "-zmqpubsequence=tcp://0.0.0.0:${APP_UMBREL_BITCOIN_ZMQ_SEQUENCE_PORT}" )
BIN_ARGS+=( "-txindex=1" )
...
export APP_UMBREL_BITCOIN_RPC_HIDDEN_SERVICE="$(cat "${rpc_hidden_service_file}" 2>/dev/null || echo "notyetset.onion")"
export APP_UMBREL_BITCOIN_P2P_HIDDEN_SERVICE="$(cat "${p2p_hidden_service_file}" 2>/dev/null || echo "notyetset.onion")"