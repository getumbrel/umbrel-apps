...
export APP_UMBREL_LIGHTNING_NODE_REST_PORT="8080"
export APP_UMBREL_LIGHTNING_NODE_DATA_DIR="${EXPORTS_APP_DIR}/data/lnd"
...
BIN_ARGS+=( "--restlisten=0.0.0.0:${APP_UMBREL_LIGHTNING_NODE_REST_PORT}" )
...
BIN_ARGS+=( "--bitcoind.rpcpass=${APP_UMBREL_BITCOIN_RPC_PASS}" )
...
export APP_UMBREL_LIGHTNING_REST_HIDDEN_SERVICE="$(cat "${rest_hidden_service_file}" 2>/dev/null || echo "notyetset.onion")"
export APP_UMBREL_LIGHTNING_GRPC_HIDDEN_SERVICE="$(cat "${grpc_hidden_service_file}" 2>/dev/null || echo "notyetset.onion")"